...
INDEX_CONFIG = {
    "weights": {
        "Adaptability_Index_weight": 0.1,
        "Solar_Energy_Index_weight": 0.15,
        "Heat_Hazard_Index_weight": 0.00,
        "Flood_Hazard_Index_weight": 0.1,
        "Heat_Vulnerability_Index_weight": 0.25,
...
def create_index_section(index_name, friendly_name):
    ...
    with st.sidebar.expander(friendly_name, expanded=False):
        ...
        initial_val = float(INDEX_CONFIG["weights"][f"{index_name}_weight"])  # Ensure float
        val = st.slider(
            "Overall Weight",
            min_value=0.0,
            max_value=1.0,
        ...
        if comp_key in COMPONENTS:
            ...
            for comp, default_w in COMPONENTS[comp_key].items():
                ...
                # Ensure the default weight is a float
